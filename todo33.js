...
const add = (e) => {
    ...
    search.value = "";
    if (todovalue.trim() === "") {
        displayMessage(`Please enter a task before saving.<br> Empty tasks cannot be added.`);
    } else if (!/^\D/.test(todovalue) && todovalue.length === 1) {
        displayMessage("Please enter a valid task.<br> Tasks cannot start with digits.");
    } else if (todovalue.length < 3) {
        displayMessage("Your task is too short.<br> Please enter at least 3 characters.");
